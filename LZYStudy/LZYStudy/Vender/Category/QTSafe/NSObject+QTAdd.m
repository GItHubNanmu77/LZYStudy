...
+ (BOOL)swizzleInstanceMethod:(SEL)srcSel tarClass:(Class)tarClass tarSel:(SEL)tarSel; {
    if (!srcSel||!tarClass||!tarSel) return NO;
    
    Method srcMethod = class_getInstanceMethod(self,srcSel);
    Method tarMethod = class_getInstanceMethod(tarClass,tarSel);
    if (!srcMethod || !tarMethod) return NO;
    method_exchangeImplementations(class_getInstanceMethod(self, srcSel),
                                   class_getInstanceMethod(tarClass,tarSel));
    return YES;
}
    ...
    if (properties) {
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            if ([self valueForKey:propertyName]) {
                if (genericMapper[propertyName] && [[self valueForKey:propertyName] isKindOfClass:[NSArray class]]) {
                    NSArray *list = [self valueForKey:propertyName];
                    [list enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                        Class subCls = genericMapper[propertyName];
                        unsigned int subPropertyCount = 0;
                        objc_property_t *subProperties = class_copyPropertyList(subCls, &subPropertyCount);
                        if (subProperties) {
                            for (unsigned int i = 0; i < subPropertyCount; i++) {
                                const char *subName = property_getName(subProperties[i]);
                                NSString *subPropertyName = [NSString stringWithUTF8String:subName];
                                if ([obj valueForKey:subPropertyName]) {
                                    NSString *key = [NSString stringWithFormat:@"%@[%@].%@", propertyName, @(idx), subPropertyName];
                                    dic[key] = [obj valueForKey:subPropertyName];
                                }
                            }
                            free(subProperties);
                        }
                    ...
                    dic[propertyName] = [self valueForKey:propertyName];
                }
            }
        }
        free(properties);
...
- (NSComparisonResult)compare:(NSObject *)object {
    return [[object yy_modelToJSONData] compare:[object yy_modelToJSONData]];
}
